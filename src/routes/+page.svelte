<script lang="ts">
    ...
    import ModalResister from "$lib/components/ModalResister.svelte";
...
<div class="container mx-auto h-screen bg-gray-100">
    <h1
        class="fixed top-0 left-0 right-0 p-2 text-center text-4xl text-white font-bold bg-blue-500"
    >
        習慣化アプリ
    </h1>
    <div class="register-area pt-20 pb-2 px-2">
        ...
        {#if registerMode}
            ...
            <form
                method="POST"
                action="?/register"
                class="p-2 m-2 bg-white rounded-sm"
            >
    ...
    <ul class="tasks-list p-2 m-4 max-h-96 overflow-y-auto bg-white rounded-sm">
        {#each _tasks as task}
            <li class="task p-2 m-2">
                <p class="task-title">
                    タスク名　　：<span class="font-bold">{task.task_name}</span
                    >
                </p>
                <p class="task-date">
